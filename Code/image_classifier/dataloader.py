...
def load_single_image(row, label_map,tolabel_row):
...
class CustomCarDataset(Dataset):
    def __init__(self, csv_file, phase='train', in_memory=False,amount=0,tolabel='brand',equallydistributed=False,ignoreLabels=[]):
        ...
        if amount > 0 and not equallydistributed:
    ...
    def preload_images(self):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            with tqdm.tqdm(total=len(rows), desc="Images to RAM", unit='image', dynamic_ncols=True) as tbar:
                for _, row in rows:
                    futures.append(executor.submit(load_single_image, row, self.label_map,self.tolabelrow))
    ...
    def __getitem__(self, idx):
        if self.in_memory:
            ...
            image, label = load_single_image(self.data.iloc[idx],self.label_map,self.tolabelrow)
